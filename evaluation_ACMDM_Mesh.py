...
def main(args):
    ...
    after_mean = np.load(pjoin(args.checkpoints_dir, args.dataset_name, args.ae_name, args.after_mean))
    after_std = np.load(pjoin(args.checkpoints_dir, args.dataset_name, args.ae_name, args.after_std))
    mesh_mean = np.load(f'utils/mesh_mean_std/{args.dataset_name}/mesh_mean.npy') # or yours
    mesh_std = np.load(f'utils/mesh_mean_std/{args.dataset_name}/mesh_std.npy') # or yours
    ...
    ae = AE_models[args.ae_model]()
    ckpt = torch.load(pjoin(args.checkpoints_dir, args.dataset_name, args.ae_name, 'model', 'latest.tar'),
                      map_location='cpu')
    model_key = 'ae'
    ae.load_state_dict(ckpt[model_key])

    ema_acmdm = ACMDM_models[args.model](input_dim=dim_pose_ae, cond_mode='text')
    ...
    for i in range(repeat_time):
        with torch.no_grad():
            ...
            best_fid, best_div, best_top1, best_top2, best_top3, best_matching, best_mm, clip_score, writer, save_now, _, _, _ = evaluation_acmdm(
                ...
                after_mean=after_mean, after_std=after_std, mesh_mean=mesh_mean, mesh_std=mesh_std,
                cond_scale=args.cfg, cal_mm=args.cal_mm, is_mesh=True,
                draw=False)
        fid.append(best_fid)
        div.append(best_div)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='ACMDM-Mesh-Flow-S-PatchSize28')
