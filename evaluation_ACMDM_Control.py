...
from models.ACMDM_ControlNet import ACMDM_ControlNet_Models
...
def main(args):
    ...
    ema_acmdm = ACMDM_ControlNet_Models[args.model](input_dim=ae.output_emb_width, cond_mode='text', need_base=False, base_checkpoint=None)
    ...
    with torch.no_grad():
        ...
        best_fid, best_div, best_top1, best_top2, best_top3, best_matching, best_mm, clip_score, writer, save_now, dist_mean, skating_score, traj_err = evaluation_acmdm(
            model_dir, eval_loader, ema_acmdm, ae, None, 0, best_fid=best_fid, clip_score_old=clip_score,
            ...
            cond_scale=args.cfg, cal_mm=args.cal_mm, is_control=True, index=args.index, intensity=args.intensity,
            draw=False)
    fid.append(best_fid)
    div.append(best_div)
    ...
    msg_final = f"\tFID: {np.mean(fid):.3f}, conf. {np.std(fid) * 1.96 / np.sqrt(1):.3f}\n" \
                f"\tDiversity: {np.mean(div):.3f}, conf. {np.std(div) * 1.96 / np.sqrt(1):.3f}\n" \
                f"\tTOP1: {np.mean(top1):.3f}, conf. {np.std(top1) * 1.96 / np.sqrt(1):.3f}, TOP2. {np.mean(top2):.3f}, conf. {np.std(top2) * 1.96 / np.sqrt(1):.3f}, TOP3. {np.mean(top3):.3f}, conf. {np.std(top3) * 1.96 / np.sqrt(1):.3f}\n" \
                f"\tMatching: {np.mean(matching):.3f}, conf. {np.std(matching) * 1.96 / np.sqrt(1):.3f}\n" \
                f"\tMultimodality:{np.mean(mm):.3f}, conf.{np.std(mm) * 1.96 / np.sqrt(1):.3f}\n\n" \
                f"\tCLIP-Score:{np.mean(clip_scores):.3f}, conf.{np.std(clip_scores) * 1.96 / np.sqrt(1):.3f}\n\n" \
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='ACMDM-Flow-S-PatchSize22-ControlNet')
    ...
    parser.add_argument("--cfg", type=float, nargs='+', default=[2.5, 1.75])
