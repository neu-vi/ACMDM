...
def main(args):
    ...
    # assuming you used my script
    checkpoint = torch.load(pjoin(model_dir.replace("_ControlNet-222", ""), 'latest.tar'), map_location='cpu')
    acmdm = ACMDM_ControlNet_Models[args.model](input_dim=ae.output_emb_width, cond_mode='text', base_checkpoint=checkpoint)
    ...
    while epoch < args.epoch:
        ...
        for i, batch_data in enumerate(train_loader):
            ...
            for kk in range(accum):
                loss, loss_control = acmdm.forward_control_loss(latent.permute(0, 3, 1, 2)[kk * accum_bs:(kk + 1) * accum_bs],
                                          conds[kk * accum_bs:(kk + 1) * accum_bs],
                                          m_lens[kk * accum_bs:(kk + 1) * accum_bs],
                                          motion[kk * accum_bs:(kk + 1) * accum_bs].permute(0, 3, 1, 2),
                                          args.control_joints, ae, (np.load(pjoin(args.checkpoints_dir, args.dataset_name, args.ae_name, args.after_mean)),
                                                                    np.load(pjoin(args.checkpoints_dir, args.dataset_name, args.ae_name, args.after_std)), mean, std))
                logs['loss'] += loss.item() / accum
                logs['loss_control'] += loss_control.item() / accum
        ...
        with torch.no_grad():
            for i, batch_data in enumerate(val_loader):
                ...
                loss, loss_control = acmdm.forward_control_loss(latent.permute(0, 3, 1, 2), conds, m_lens,
                                                                motion.permute(0, 3, 1, 2), args.control_joints, ae, (np.load(pjoin(args.checkpoints_dir, args.dataset_name, args.ae_name, args.after_mean)),
                                                                    np.load(pjoin(args.checkpoints_dir, args.dataset_name, args.ae_name, args.after_std)), mean, std))
                loss = loss + loss_control
...
if __name__ == "__main__":
    ...
    parser.add_argument('--name', type=str, default='ACMDM_Flow_S_PatchSize22_ControlNet')
    ...
    parser.add_argument('--control_joints', default=[0, 10, 11, 15, 20, 21], nargs="+", type=int)
